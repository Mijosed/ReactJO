...
import { HeaderHome, MapSection, Footer, HomeTitle, SportSection } from "../components/Components.js";
...
export class HomePage extends Component {
  ...
  async componentDidMount() {
    try {
      ...
      olympicSites.forEach((site) => {
        const marker = L.marker(site.coords).addTo(map).bindPopup(site.name).openPopup();
        marker.on("click", (event) => this.openMenu(event));
      });
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition((position) => {
          ...
          L.marker([latitude, longitude], { icon: customIcon }).addTo(map).bindPopup("Je suis géolocalisé(e) !").openPopup();
  ...
  render() {
    ...
    return {
      tag: "div",
      children: [this.headerHome.render(), this.titleElementSites.render(), this.mapElement.render(), this.sportsSection.render(), this.footerElement.render()],
    };
  }
}
